...
# Выбор фреймворка:
## CustomTkinter - он основан на Tkinter (встроен в Python), выглядит как современные приложения.
# План реализации:
## 1. Установка CustomTkinter:
```` python
pip install customtkinter
````
## 2. Создание gui_app.py (решение):
```` python
# БЛОК 1: ИМПОРТЫ
...
1. Главное окно (LabApp)
2. Система вкладок (CTkTabview)
3. Поля ввода (CTkEntry, CTkTextbox)
4. Кнопки управления
5. Область вывода результатов
# Описание проделанной работы:
# Реализация вкладок лабораторных работ
## Лабораторная работа №4 (Рекурсия и итерация)
## Особенности реализации:
1) Выпадающий список на 4 варианта (распаковка рекурсивная/итеративная, последовательность рекурсивная/итеративная)
2) Динамическое изменение поля ввода в зависимости от выбора:
3) Для распаковки: поле для Python-выражения (пример: `[None, [1, ({2, 3}, {'foo': 'bar'})]]`)
4) Для последовательности: поле для целого числа (пример: 5)
5) Обработка ошибок: try/except с выводом в поле результата
## Лабораторная работа №5 (Замыкания и декораторы)
## Особенности реализации:
1) Выпадающий список на 2 варианта
2) Динамическое отображение панели выбора файла (для замыкания) / скрытие (для декоратора)
3) Интеграция с системным диалогом выбора файла (filedialog.askopenfilename)
4) Для декоратора: демонстрация с примером функции example_func
## Особенность декоратора в GUI:
## Поскольку декоратор log_c выводит логи в консоль (а не в GUI), в интерфейсе выводится пояснение:
```` text 
"Декоратор отработал. Проверьте консоль для просмотра логов."
````
## Лабораторная работа №6 (Генератор простых чисел)
## Особенности реализации:
1) Два поля ввода для нижней и верхней границ
2) Проверка: нижняя граница не может быть больше верхней
3) Вывод результата: список простых чисел и их количество
# Обработка ошибок
## В приложении реализована многоуровневая обработка ошибок:
### Неверный ввод данных	-> `try/except с Exception`	 -> Вывод сообщения в поле результата
### Ошибка преобразования типа -> `ValueError` -> Сообщение "введите целые числа"
### Файл не найден -> `FileNotFoundError` -> Сообщение "файл не найден"
### Рекурсия превышена -> `RecursionError` -> Сообщение о глубине рекурсии
### Пустой файл -> Проверка `if not lines` -> Сообщение "файл пуст"
## Интеграция с пакетом `rare_package`:
### GUI приложение импортирует те же самые функции из пакета `rare_package`, что и CLI версия
## Преимущества такого подхода:
1) Единая бизнес-логика - обе версии (CLI и GUI) используют один и тот же код
2) Лёгкость поддержки - изменения в логике достаточно внести в одном месте
3) Модульность - интерфейс и логика разделены
