...
* Файл закрывается
# Создание функции `get_file_reader(filename)`
...
````
# Описание проделанной работы:
# Импортируется модуль `time` для работы со временем
# Создание декоратора `log_c`
* Декоратор `log_c` принимает функцию func в качестве аргумента и возвращает обёртку wrapper, которая добавляет логирование к исходной функции.
...
* Декоратор применяется к функции add с помощью синтаксиса `@log_c`
* Декоратор применяется к функции greet с помощью синтаксиса `@log_c`
* Теперь при вызове этих функций автоматически выполняется логирование
# Создаем тестрируемые функции
# Запускаем
